package com.makr.jumpnbump 
{
	...
	public class AI
	{
		...
		private var _tileMapDimensions:Point = new Point(22, 16);
		...
		private function toIndex(X:int, Y:int):int	{ return Y * _tileMapDimensions.x + X; }
		private function toCoordinates(Index:int):Point	{ return new Point(Index % _tileMapDimensions.x, int(Index / _tileMapDimensions.x)); }
		...
		public function AI(RabbitIndex:uint, LevelTileMap:FlxTilemap) 
		{
			...
			for (x = 0; x < _tileMapDimensions.x; x++) 
			{
				for (y = 0; y < _tileMapDimensions.y; y++) 
				{
			...
			// fill map with information on which tiles a player can walk on 
			
			for (x = 0; x < 22; x++) 
			{
				for (y = 0; y < 15; y++) 
				{
					if (LevelTileMap.getTileByIndex(toIndex(x, y)) < LevelTileMap.collideIndex &&		// current Tile is non colliding
						LevelTileMap.getTileByIndex(toIndex(x, y + 1)) >= LevelTileMap.collideIndex)	// and Tile below is noncolliding
						_tileMap[toIndex(x, y)].walkable = true;										// mark as walkable
					
					// special case for water
					if (LevelTileMap.getTileByIndex(toIndex(x, y)) == 0 &&		// current Tile is VOID 
						LevelTileMap.getTileByIndex(toIndex(x, y + 1)) == 1)	// and Tile below is not WATER
						_tileMap[toIndex(x, y + 1)].walkable = true;				// mark water surface as walkable
				}
				if (LevelTileMap.getTileByIndex(toIndex(x, 15)) == 0)			// can walk on the bottom of the level, where there are no tiles below
					_tileMap[toIndex(x, 15)].walkable = true;
			}
		}
		...
		public function findPath():Array
		{
			...
			var startIndex:int = _currentTile;
			var endIndex:int = _targetTile;
			...
			while (!finishedPathfinding)
			{
				...
				if (currentTilePosition.y != 0)
				{
					...
					if (currentTilePosition.x != _tileMapDimensions.x - 1)
						adjacentTiles.push(toIndex(currentTilePosition.x + 1, 	currentTilePosition.y - 1));
				...
				// bottom row
				if (currentTilePosition.y != _tileMapDimensions.y - 1)
				{
					if (currentTilePosition.x != 0)
						adjacentTiles.push(toIndex(currentTilePosition.x - 1, 	currentTilePosition.y + 1));
					adjacentTiles.push(toIndex(currentTilePosition.x, 			currentTilePosition.y + 1));
		...
		{
			var newCurrent:int = toIndex(limit(CurrentTile.x, 0, _tileMapDimensions.x-1), 	limit(CurrentTile.y, 0, _tileMapDimensions.y-1));
			var newTarget:int =  toIndex(limit(TargetTile.x, 0, _tileMapDimensions.x - 1), 	limit(TargetTile.y, 0, _tileMapDimensions.y - 1));
		...
		public function drawDebugImage():void
		{
			...
			var debugImage:BitmapData = new BitmapData(_tileMapDimensions.x, _tileMapDimensions.y);
			...
			for (y = 0; y < _tileMapDimensions.y ; y++) 
				for (x = 0; x < _tileMapDimensions.x ; x++) 
				{
					if (!_tileMap[toIndex(x, y)].free)
						color = 0xffffff;
					else if (_tileMap[toIndex(x, y)].walkable)
						color = 0x222222;
					else
						color = 0x000000;
			...
			FlxG.buffer.draw(debugImage, debugImageMatrix, new ColorTransform(1, 1, 1, 0.6), "normal");
		}
	}
